    ...
    private data class Spell(val target: ItemEntity) : RenderedSpell {
        override fun cast(ctx: CastingContext) {
            ...
            for (i in 0..item.count){
                val results: MutableList<ItemStack>? =  smeltResult(item.item, ctx) ;
                if (results != null) {
        ...
        fun smeltResult(item: Item, ctx: CastingContext): MutableList<ItemStack>? {
            val recipeType = RecipeType.SMELTING;
            val itemStack = ItemStack(item, 1)
            val recipe: Optional<SmeltingRecipe> = ctx.world.recipeManager.getRecipeFor(
                    recipeType, SimpleContainer(itemStack),
                    ctx.world)
            if (!recipe.isPresent) return null
            //if (result.isEmpty()) return null
            return mutableListOf(recipe.get().resultItem)
        }
