    ...
    private data class Spell(val target: ItemEntity) : RenderedSpell {
        override fun cast(ctx: CastingContext) {
            ...
            for (i in 0..item.count){
                val results: MutableList<ItemStack>? =  pressResult(item.item, ctx) ;
                if (results != null) {
        ...
        fun pressResult(item: Item, ctx: CastingContext): MutableList<ItemStack>? {
            val recipeType: IRecipeTypeInfo = AllRecipeTypes.PRESSING
            val itemStack = ItemStack(item, 1)
            val recipe: Optional<PressingRecipe> = ctx.world.recipeManager.getRecipeFor(
                    recipeType.getType(), SimpleContainer(itemStack),
                    ctx.world)
