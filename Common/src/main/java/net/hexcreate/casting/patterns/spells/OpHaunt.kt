    ...
    private data class Spell(val target: ItemEntity) : RenderedSpell {
        override fun cast(ctx: CastingContext) {
            ...
            for (i in 0..item.count){
                val results: MutableList<ItemStack>? =  hauntResult(item.item, ctx) ;
                if (results != null) {
        ...
        fun hauntResult(item: Item, ctx: CastingContext): MutableList<ItemStack>? {
            val recipeType: IRecipeTypeInfo = AllRecipeTypes.HAUNTING
            val itemStack = ItemStack(item, 1)
            val hauntingWrapper = HauntingWrapper()
            hauntingWrapper.setStackInSlot(0, itemStack)
            val recipe: Optional<HauntingRecipe> = ctx.world.recipeManager.getRecipeFor(
                    recipeType.getType(), hauntingWrapper,
                    ctx.world)
